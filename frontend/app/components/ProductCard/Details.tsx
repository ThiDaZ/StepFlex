...
import { colors, sizeOptions, paymentImages } from "@/data/singleProducts";
...
export default function Details(productID: detailsProps) {
  ...
  const handleColor = (color: colorProps) => {
    const updatedColor = colors.filter(
      (elm) => elm.value.toLowerCase() == color.value.toLowerCase()
    )[0];
    if (updatedColor) {
